<x-app-layout>
        ...
        <div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                ...
                <div
                    ...
                    <div
                        class="px-6 py-5 border-b border-slate-200 bg-white/90 backdrop-blur-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <span
                                class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-semibold text-indigo-700 ring-1 ring-inset ring-indigo-700/10">
                            ...
                            <table class="min-w-full divide-y divide-slate-200">
                                <thead class="bg-slate-50">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="px-5 py-4 text-center text-xs font-semibold uppercase tracking-wider text-slate-600 w-36">
                                            Aksi
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    @forelse ($mahasiswa as $m)
                                        <tr class="hover:bg-slate-50/80 transition-colors duration-150 group">
                                            <td
                                                class="whitespace-nowrap px-5 py-4 text-sm font-mono font-medium text-slate-700">
                                                {{ $m->nim }}
                                            </td>
                                            <td
                                                class="whitespace-nowrap px-5 py-4 text-sm font-semibold text-slate-800">
                                                {{ $m->nama }}
                                            </td>
                                            <td class="whitespace-nowrap px-5 py-4 text-sm text-slate-600">
                                                <span
                                                    class="inline-flex rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-200">
                                            ...
                                            <td class="whitespace-nowrap px-5 py-4 text-sm text-center">
                                                <div class="flex items-center justify-center gap-2">
                                                    <a href="{{ route('mahasiswa.edit', $m->id) }}"
                                                        class="inline-flex items-center justify-center rounded-lg bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-700 ring-1 ring-inset ring-amber-300 hover:bg-amber-100 transition-all duration-200 group-hover:shadow-sm">
                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                            class="h-3.5 w-3.5 mr-1 -ml-0.5" fill="none"
                                                    ...
                                                    <form action="{{ route('mahasiswa.destroy', $m->id) }}"
                                                        ...
                                                        onsubmit="return confirm('Yakin ingin menghapus mahasiswa {{ addslashes($m->nama) }}?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="inline-flex items-center justify-center rounded-lg bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-700 ring-1 ring-inset ring-red-300 hover:bg-red-100 transition-all duration-200 group-hover:shadow-sm">
                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                class="h-3.5 w-3.5 mr-1 -ml-0.5" fill="none"
                                                ...
                                                <div class="flex flex-col items-center justify-center gap-3">
                                                    <div class="rounded-full bg-slate-100 p-4">
                                                        <svg class="h-8 w-8 text-slate-400" fill="none"
                                                            ...
                                                            stroke-width="1.5">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586M4 13v5m8-3v6m0 0H7m5 0h5" />
                                                        </svg>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <p class="text-sm text-slate-400 mt-1">Silakan klik tombol
                                                            "Tambah Mahasiswa" untuk mulai menambahkan</p>
                                                    </div>
                                                    <a href="{{ route('mahasiswa.create') }}"
                                                        class="mt-2 inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700">
                                                        + Tambah Sekarang
                                                    </a>
                                                </div>
                                            </td>
    ...
    <a href="{{ route('mahasiswa.export-excel') }}" target="_blank" class="btn btn-success">
        Export Excel
    </a>
</x-app-layout>
